...
const { log, npm, Package, exec, locale } = require('@fast-learn/utils')
  ...
  program
    ...
    .action(
      async ({
        ...
        const packageName = '@fast-learn/publish'
  ...
  program
    ...
    .action(
      async ({ packagePath, region, bucket, ossAccessKey, ossSecretKey }) => {
        const packageName = '@fast-learn/replace'
        const packageVersion = '1.0.0'
        await execCommand(
          { packagePath, packageName, packageVersion },
...
function checkNodeVersion() {
  ...
  if (!semver.gte(process.version, LOWEST_NODE_VERSION)) {
    throw new Error(
      colors.red(
        `fast-learn 需要安装 v${LOWEST_NODE_VERSION} 以上版本的 Node.js`
      )
    )
  }
}
